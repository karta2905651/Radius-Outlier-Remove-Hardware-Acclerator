...
static void start_transfer(struct dma_proxy_channel *pchannel_p)
{
	...
	int bdindex = pchannel_p->bdindex;
        ...
        printk(KERN_INFO
            "[DEBUG] start_transfer() called\n"
            "        bdindex           = %d\n"
            "        dma_handle (phys) = %pad\n"
            "        buffer length     = %d\n"
            "        direction         = %s\n",
            bdindex,
            &pchannel_p->bdtable[bdindex].dma_handle,
            pchannel_p->buffer_table_p[bdindex].length,
            pchannel_p->direction == DMA_MEM_TO_DEV ? "DMA_MEM_TO_DEV" : "DMA_DEV_TO_MEM");
	...
	chan_desc = dmaengine_prep_slave_single(pchannel_p->channel_p, pchannel_p->bdtable[bdindex].dma_handle,  // DMA 總線位址
                                                pchannel_p->buffer_table_p[bdindex].length, pchannel_p->direction, flags);					
...
static void test(struct dma_proxy *lp)
{
	 ...
	 */
	printk(KERN_INFO "[TEST] Step 1: Initializing buffers\n");
	 ...
	 * thread
	 */
	printk(KERN_INFO "[TEST] Step 2: Scheduling TX transfer\n"); 
	INIT_WORK(&lp->work, tx_test);
	schedule_work(&lp->work);

	/* Receive the data that was just sent and looped back
	 */
	printk(KERN_INFO "[TEST] Step 3: Setting RX buffer length\n"); 
	lp->channels[RX_CHANNEL].buffer_table_p->length = TEST_SIZE;
	lp->channels[TX_CHANNEL].bdindex = 0;

        printk(KERN_INFO "[TEST] Step 4: Starting RX transfer\n");
	start_transfer(&lp->channels[RX_CHANNEL]);
	
	printk(KERN_INFO "[TEST] Step 5: Waiting RX transfer to complete\n");
	 ...
	 */
	printk(KERN_INFO "[TEST] Step 6: Comparing buffers\n");
	for (i = 0; i < TEST_SIZE / sizeof(unsigned int); i++)
...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	...
	for (bd = 0; bd < BUFFER_COUNT; bd++) {
		...
		printk(KERN_INFO "\t%d: %px\n",bd,(void*)pchannel_p->bdtable[bd].dma_handle);
    }
...
static int dma_proxy_probe(struct platform_device *pdev)
{
    ...
    * from the device tree
	...
	for (i = 0; i < lp->channel_count; i++) {
	        ...
	        u32 dir = (strstr(lp->names[i], "rx") != NULL)
                          ? DMA_DEV_TO_MEM    // RX: device -> memory
                          : DMA_MEM_TO_DEV;   // TX: memory -> device
                          
		printk(KERN_INFO "Creating channel %s (dir=%s)\n", lp->names[i] , (dir == DMA_DEV_TO_MEM) ? "DEV->MEM" : "MEM->DEV");
		rc = create_channel(pdev, &lp->channels[i], lp->names[i], dir);
